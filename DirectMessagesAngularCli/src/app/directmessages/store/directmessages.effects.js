...
let DirectMessagesEffects = class DirectMessagesEffects {
  constructor(directMessagesService, actions$) {
    ...
    this.sendDirectMessage$ = this.actions$.pipe(
      ofType(directMessagesAction.SEND_DIRECT_MESSAGE),
      switchMap((action) => {
        ...
        return of(
          new directMessagesAction.SendDirectMessageActionComplete(
            action.message
          )
        );
      })
    );
    this.Leave$ = this.actions$.pipe(
      ofType(directMessagesAction.LEAVE),
      switchMap(() => {
        this.directMessagesService.leave();
        return of(new directMessagesAction.LeaveSent());
      })
    );
    this.Join$ = this.actions$.pipe(
      ofType(directMessagesAction.JOIN),
      switchMap(() => {
...
__decorate(
  [Effect(), __metadata("design:type", Observable)],
  DirectMessagesEffects.prototype,
  "sendDirectMessage$",
  void 0
);
__decorate(
...
DirectMessagesEffects = __decorate(
  [
    Injectable(),
    __metadata("design:paramtypes", [DirectMessagesService, Actions]),
  ],
